#!/usr/bin/env python
"""Daily NBA Betting Recommendations Pipeline - Experiment 2 Model

This script uses the Experiment 2 model (home_ftr + role indicators with standard loss).
This model was selected based on its strong performance:
- 60.50% ATS accuracy on 2025-2026 season
- Includes home_ftr feature to capture free throw rate regime changes
- Includes favorite/underdog role indicators (home/away x favorite/underdog)
...
3. Generates margin predictions using Experiment 2 model
4. Analyzes predictions for betting recommendations
5. Emails results via SendGrid API

Usage:
    python daily_betting_recommendations_exp2.py --api-key YOUR_SENDGRID_API_KEY
    
    # Dry run (skip email)
    python daily_betting_recommendations_exp2.py --dry-run
    
    # Custom date (Eastern time)
    python daily_betting_recommendations_exp2.py --date 2025-11-10 --api-key YOUR_KEY
"""
...
def format_email_body(date_str: str, recommendations: List[Dict]) -> str:
    ...
    for rec in filtered_recs:
        ...
        # Case 2: Home team narrowly favored (spread between -4 and 0), model picks home
        # Spread must be > -4 and < 0 (home small favorite)
        elif spread > -4 and spread < 0 and pick_side == 'home':
            is_best_value = True
    ...
    html += """
                <div class="footer">
                    ...
                    <p>Model: Experiment 2 (home_ftr + role indicators) | """ + datetime.now().strftime("%Y-%m-%d %H:%M:%S %Z") + """</p>
                </div>
            </div>
        </div>
    </body>
...
def main():
    parser = argparse.ArgumentParser(
        description="Daily NBA betting recommendations pipeline (Experiment 2 Model)",
    ...
    logger.info("Model: Experiment 2 (home_ftr + role indicators)")
    logger.info("=" * 70)
    logger.info("")
    
    # Define file paths
    data_file = f"data/games_future_{date_str}_norm.jsonl"
    predictions_file = f"predictions/predictions_exp2_{date_str}.csv"
    ...
    # STEP 2: Generate predictions using Experiment 2 model
    # The predict_margin.py script will automatically use the correct features
    # based on the model's config (home_ftr + role indicators)
    cmd = [
        sys.executable,
        "predict_margin.py",
        "--data", data_file,
    ...
    success = run_step("Generate Predictions (Experiment 2 Model)", cmd)
    if not success:
    ...
    if args.dry_run:
        ...
        preview_file = Path("predictions") / f"email_preview_exp2_{date_str}.html"
        ...
        subject = f"NBA Betting Recommendations - {date_str} (Exp2 Model)"
        html_content = format_email_body(date_str, recommendations)
        ...
        if success:
            ...
            backup_file = Path("predictions") / f"email_backup_exp2_{date_str}.html"
