...
class CalibrationConfig:
    ...
    margin_low: int = -60
    margin_high: int = 60
    ats_bin_edges: Tuple[float, ...] = (0.0, 2.0, 4.0, 6.0, 10.0, 40.0)
...
class Calibrator:
    def __init__(self, config: CalibrationConfig | None = None) -> None:
        ...
        self.ats_isos: Optional[List[IsotonicRegression]] = (
            [IsotonicRegression(out_of_bounds="clip") for _ in range(len(self.ats_bin_edges) - 1)]
    ...
    def fit(
        ...
        if self.ats_isos is not None and market_home is not None and market_away is not None:
            ...
            # Prepare per-bin fit data
            for b in range(len(self.ats_bin_edges) - 1):
                ...
                if idx.size == 0:
                    continue
                s_sel = spreads[idx]
                # Predicted unconditional cover probability (home)
                pred_cover = np.empty(idx.size, dtype=float)
                for k, i in enumerate(idx):
                    s = s_sel[k]
                    pred_cover[k] = pmf[i, m_vals > -s].sum()
                # Actual cover indicator; treat push as 0.5 for monotone fit stability
                y = np.full(idx.size, 0.0, dtype=float)
                margins = margin_outcomes[idx]
                y[margins > -s_sel] = 1.0
                y[np.isclose(margins, -s_sel)] = 0.5
                try:
                    self.ats_isos[b].fit(pred_cover, y)
                except Exception:
                    # If degenerate, fall back to identity by fitting endpoints
                    self.ats_isos[b] = IsotonicRegression(out_of_bounds="clip")
                    self.ats_isos[b].fit(np.array([0.0, 1.0]), np.array([0.0, 1.0]))
    ...
    def calibrate_margin_cdf(self, margin_cdf: np.ndarray) -> np.ndarray:
        # Convert CDF back to PMF, apply temperature, then re-CDF and apply isotonic
        pmf = np.diff(
            np.concatenate([np.zeros((len(margin_cdf), 1)), margin_cdf], axis=1),
            axis=1,
        )
    ...
    def calibrate_cover_prob(self, cover_probs: np.ndarray, spreads: np.ndarray) -> np.ndarray:
        ...
        for b in range(len(ats_isos)):
            ...
            if not np.any(mask):
                continue
            iso = ats_isos[b]
            calibrated[mask] = np.clip(iso.transform(cover_probs[mask]), 0.0, 1.0)
