"""
Feature selection for XGBoost margin prediction.

This script systematically tests different feature subsets to find the optimal
combination that maximizes test set ATS accuracy while avoiding overfitting.
...
3. Feature Ablation: Tests removing bottom N% of features by importance

Usage:
    python select_features_xgboost.py --model artifacts/margin_xgboost_no_diff \\
                                       --train-data data/games_train_with_players_90_norm.jsonl \\
                                       --test-data data/games_predict_2024_2025_with_players_norm.jsonl \\
                                       --output reports/feature_selection
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Feature selection for XGBoost")
    ...
    parser.add_argument("--output", type=str, default="reports/feature_selection",
...
def permutation_importance_ats(
    ...
    y_pred = model.predict(x, baseline)
    baseline_acc = calculate_ats_accuracy(y_pred, y, spreads)
    ...
    for i, feat_name in enumerate(tqdm(feature_names, desc="Features")):
        ...
        for _ in range(n_permutations):
            ...
            y_pred_perm = model.predict(x_permuted, baseline)
            acc_perm = calculate_ats_accuracy(y_pred_perm, y, spreads)
...
def train_and_evaluate_subset(
    ...
    # Train model
    model = MarginXGBoostModel(config)
    model.fit(x_train, train_batch.y_margin, train_batch.baseline_margin)
    
    # Evaluate on train and test
    train_pred = model.predict(x_train, train_batch.baseline_margin)
    test_pred = model.predict(x_test, test_batch.baseline_margin)
    
    train_ats = calculate_ats_accuracy(train_pred, train_batch.y_margin, train_batch.market_spread_home)
    test_ats = calculate_ats_accuracy(test_pred, test_batch.y_margin, test_batch.market_spread_home)
    
    train_mae = np.abs(train_pred - train_batch.y_margin).mean()
    test_mae = np.abs(test_pred - test_batch.y_margin).mean()
...
def ablation_study(
    ...
    """
    Test removing bottom N% of features by importance.
    """
    results = []
...
def main():
    ...
    include_diff_features = model_cfg.get('include_diff_features', False)
    ...
    # Get baseline importance from trained model
    print("\nGetting baseline feature importance...")
    base_importance_dict = model.get_feature_importance(importance_type='gain')
    
    # Convert to our feature naming
    base_importance = {}
    for feat_name, score in base_importance_dict.items():
        if feat_name.startswith('f'):
            idx = int(feat_name[1:])
            if idx < len(feature_names):
                base_importance[feature_names[idx]] = score
    ...
    train_pred = model.predict(train_batch.x, train_batch.baseline_margin)
    test_pred = model.predict(test_batch.x, test_batch.baseline_margin)
    baseline_train_ats = calculate_ats_accuracy(train_pred, train_batch.y_margin, train_batch.market_spread_home)
    baseline_test_ats = calculate_ats_accuracy(test_pred, test_batch.y_margin, test_batch.market_spread_home)
    ...
    perm_df = pd.DataFrame([
        {'feature': feat, 'permutation_importance': score, 'gain_importance': base_importance.get(feat, 0)}
        for feat, score in sorted_perm
    ])
    perm_df.to_csv(output_dir / "permutation_importance.csv", index=False)
    
    # 2. Recursive Feature Elimination
    config = MarginXGBoostConfig(**model_cfg)
    ...
    for feat in best_overall['features']:
        gain = base_importance.get(feat, 0)
        perm = perm_importance.get(feat, 0)
        print(f"  {feat}: gain={gain:.2f}, perm_importance={perm:.4f}")
    ...
    with open(output_dir / "feature_selection_report.txt", 'w') as f:
        f.write("="*70 + "\n")
        f.write("XGBOOST FEATURE SELECTION REPORT\n")
        f.write("="*70 + "\n\n")
        ...
        for feat in best_overall['features']:
            gain = base_importance.get(feat, 0)
            perm = perm_importance.get(feat, 0)
            f.write(f"  {feat}: gain={gain:.2f}, perm_importance={perm:.4f}\n")
